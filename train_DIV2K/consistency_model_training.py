...
class LitConsistencyModel(LightningModule):
    def __init__(
        ...
        self.E.load_state_dict(torch.load(f'd:\\Python\\SemCom\\saved_model\\v_encoder_dc_model_of_DIV2K.pth'))
        for param in self.E.parameters():
            param.requires_grad = False  
        self.E.eval()
        
        self.G = Generator_DC(input_dim = optc.latent_dim).cuda()
        self.G.load_state_dict(torch.load(f'd:\\Python\\SemCom\\saved_model\\generator_dc_model_of_DIV2K.pth'))
        for param in self.E.parameters():
            param.requires_grad = False  
        self.G.eval()        
