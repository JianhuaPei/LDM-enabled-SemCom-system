...
class LitConsistencyModel(LightningModule):
    def __init__(
        ...
        self.E.load_state_dict(torch.load(f'd:\\Python\\SemCom\\saved_model\\v_encoder_dc_model_of_AFHQ.pth'))
        for param in self.E.parameters():
            param.requires_grad = False  
        self.E.eval()
        
        self.G = Generator_DC(input_dim = optc.latent_dim).cuda()
        self.G.load_state_dict(torch.load(f'd:\\Python\\SemCom\\saved_model\\generator_dc_model_of_AFHQ.pth'))
        for param in self.E.parameters():
            param.requires_grad = False  
        self.G.eval()    
...
def run_training(config: TrainingConfig) -> None:
    ...
    transforms_train = transforms.Compose([transforms.Resize(optc.inputdata_size_H), transforms.CenterCrop(optc.inputdata_size_H), transforms.PILToTensor(), transforms.ConvertImageDtype(torch.float), transforms.Normalize(mean=[0.5,0.5,0.5],std=[0.5,0.5,0.5])]) 
    ...
    train_loader = torch.utils.data.DataLoader(
        dataset = my_dataset,
        batch_size=optc.batch_size_diffusion,
        shuffle=True,
    )    
    ...
    losses = np.array([loss.detach().cpu().numpy() for loss in lit_cm.loss_history])
    plt.figure()
    plt.plot(range(1, len(losses) + 1), losses, linestyle='-', color='b', linewidth=2.0)
    plt.title('LPIPS Loss of AFHQ over Epochs')
    plt.xlabel('Epoch')
    plt.ylabel('LPIPS Loss')
    plt.xlim(-1, len(losses))
    plt.savefig('d:\\Python\\SemCom\\lpips_loss_plot.svg')  
    plt.close()     
