...
class Diffusion(pl.LightningModule):
    def __init__(self, cfg=None):
        ...
        if cfg.diffusion.preconditioning == 'vp':
            ...
            for param in self.net_ema.parameters():
                param.requires_grad = False
            self.net_ema.load_state_dict(copy.deepcopy(self.net.state_dict()))  
            self.G = Generator_DC(input_dim = opt.latent_dim).cuda()
    ...
    def training_step(self, batch, _):
        images, _ = batch
        images,mean_value, std_value = self.E(images)
        images = images.reshape(images.shape[0],1,8,8)
        loss = self.loss_fn(net=self.net, net_ema=self.net_ema,  decoder=self.G, images=images, k=self.global_step).mean()
